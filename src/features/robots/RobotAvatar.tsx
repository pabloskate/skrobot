...
export default function RobotAvatar({ robot, size = 96, pose = 'idle' }: Props) {
  ...
  const figure = (
    <g>
      ...
      {/* belly plate */}
      <rect x="39" y="58" width="18" height="14" rx="4" fill={accent} stroke={STROKE} strokeWidth="3" />
    </g>
  );
  ...
  const board = (x: number, y: number, rot = 0) => (
    <g transform={`translate(${x} ${y}) rotate(${rot})`}>
      <path
        ...
        strokeLinecap="round"
      />
      <circle cx="16" cy="10" r="3.8" fill={accent} stroke={STROKE} strokeWidth="2.5" />
      <circle cx="48" cy="10" r="3.8" fill={accent} stroke={STROKE} strokeWidth="2.5" />
    </g>
  );
