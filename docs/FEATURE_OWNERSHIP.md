...
| `analytics` | full-stack feature | Gameplay event semantics, strict privacy-safe contracts, anonymous installation identity, offline delivery, and deduplicated ingestion | `@/features/analytics`, `features/analytics/server/*` | `/api/analytics` | localStorage delivery queue, D1 event table | `npm test`, `npm run lint` |
...
| `records` | client persistence feature | W/L records, capped game log, player trick marks, proven-trick derivation, per-trick attempt stats | `@/features/records` | none today | localStorage; D1 candidate | `npm test` |
